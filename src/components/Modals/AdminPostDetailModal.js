        ...
        <div className="relative w-full max-w-4xl mx-auto bg-white rounded-lg shadow-2xl flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center justify-between p-5 border-b border-solid border-blueGray-200 rounded-t bg-blueGray-50">
            <h3 className="text-xl font-bold text-blueGray-700 uppercase">
              ...
              {t('admin_review_post', 'Duyệt bài đăng tuyển dụng')}
            </h3>
            <button
            ...
            <div className="flex flex-wrap">
              <div className="w-full lg:w-8/12 pr-4 border-r border-blueGray-100">
                ...
                <div className="flex items-center text-sm text-blueGray-500 mb-4">
                  ...
                  <span className="font-semibold">{post.companyName || t('unknown_company', 'Công ty ẩn danh')}</span>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-col gap-2">
                    {post.skills && post.skills.length > 0 ? (
                      ...
                      <p className="text-xs italic text-blueGray-400">{t('no_skills_required', 'Không yêu cầu kỹ năng cụ thể')}</p>
                    )}
                  </div>
                </div>

                <div className="bg-blueGray-100 p-4 rounded text-xs text-blueGray-600 space-y-2">
                  <p><strong>{t('post_id', 'Mã bài đăng')}:</strong> <span className="text-blueGray-800">{post.id}</span></p>
                  <p><strong>{t('created_at', 'Ngày tạo')}:</strong> {new Date(post.createdAt).toLocaleDateString('vi-VN')}</p>
                  <p><strong>{t('current_status', 'Trạng thái hiện tại')}:</strong> 
          ...
          <div className="flex items-center justify-end p-6 border-t border-solid border-blueGray-200 rounded-b bg-blueGray-50">
            ...
            >
              <i className="fas fa-times mr-2"></i> {t('reject', 'Từ chối')}
            </button>
            ...
            >
              <i className="fas fa-check mr-2"></i> {t('approve', 'Phê duyệt')}
            </button>
          </div>
        </div>
      </div>
